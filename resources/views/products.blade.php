<x-app-layout>
    ...
    <!-- Hero Banner Section -->
    <div class="relative bg-gradient-to-r from-green-500 to-green-700 overflow-hidden">
...
<div class="bg-white border-b">
    <div class="max-w-7xl mx-auto">
        <nav class="flex justify-between items-center py-4">
            <div class="flex gap-4 category-nav w-full justify-center lg:w-auto flex-wrap">
                <a href="{{ route('products') }}"
                   class="flex items-center gap-2 px-5 py-2 rounded-full border transition-all duration-200
                          {{ !$categoryId ? 'bg-green-500 text-white border-green-500 shadow-lg font-bold' : 'bg-white text-gray-600 hover:bg-green-50 border-gray-300' }}">
                    All
                </a>
                @foreach ($categories as $category)
                    <a href="{{ route('products', ['category_id' => $category->id]) }}"
                       class="flex items-center gap-2 px-5 py-2 rounded-full border transition-all duration-200
                              {{ $categoryId == $category->id ? 'bg-green-500 text-white border-green-500 shadow-lg font-bold' : 'bg-white text-gray-600 hover:bg-green-50 border-gray-300' }}">
                        {{ $category->name }}
                    </a>
                @endforeach
            </div>
